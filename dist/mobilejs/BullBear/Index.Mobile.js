            ...
            if (data != null) {
                ...
                if (result.objLstWatchList.length > 0) {
                    for (var i = 0; i < result.objLstWatchList.length; i++) {
                        ...
                        if (i == 0) {
                            $('.favorite1').html('<span class="sensex">' + result.objLstWatchList[0].ScriptTradingSymbol + '&nbsp </span><span class="sensex-price" style=" display: inline-flex;"> ' + result.objLstWatchList[0].LastPrice.toFixed(1) + '&nbsp&nbsp ' + perCentageHtml + '</span>');
                        }
                        if (i == 1) {
                            $('.favorite2').html('<span class="nifty">' + result.objLstWatchList[1].ScriptTradingSymbol + '&nbsp</span><span class="nifty-price" style=" display: inline-flex;"> ' + result.objLstWatchList[1].LastPrice.toFixed(1) + '&nbsp&nbsp ' + perCentageHtml + '</span>');
                        }
                    }
                }
            }
...
function SetWatchTradeDetails(item) {
    ...
    var buyButton = '<div tabindex="-1" style="display:none;" class="b-btn"><button class="btn-buy" id="' + btnBuyid + '" onclick="buySellPopUp(' + item.ScriptCode + ',1,' + symbolParam + ',' + item.WID + ',' + item.LastPrice + ',' + ScriptInstrumentType + ',' + ScriptExchange + ',' + qty + ',' + item.ScriptLotSize + ',' + item.high + ',' + item.low + ',' + item.LastPrice + ')" type="button" class="btn btn-success btn-sm btn-buy">B </button> ';
    var sellButton = '<button class="btn-sell" id="' + btnSellid + '" onclick="buySellPopUp(' + item.ScriptCode + ',2,' + symbolParam + ',' + item.WID + ',' + item.LastPrice + ',' + ScriptInstrumentType + ',' + ScriptExchange + ',' + qty + ',' + item.ScriptLotSize + ',' + item.high + ',' + item.low + ',' + item.LastPrice+ ')" type="button" class="btn btn-danger btn-sm btn-sell"> S </button> ';
    ...
    html = '<div class="row watchlistRowView" style="border-bottom: 1px solid #ddd;" id="' + item.ScriptCode + '" data-scriptType="' + item.ScriptType + '"  data-scripttradingsymbol="' + item.ScriptTradingSymbol + '" data-scriptexchange="' + item.ScriptExchange + '">' +
